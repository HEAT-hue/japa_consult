...
export const Toast: React.FC<ToastProp> = ({ error, desc, action }) => {
    return (
        <>
            <div
                onClick={action}
                className={`h-[50px] w-[80vw]  max-w-[400px] py-[0.7rem] fixed top-[4.6rem] right-5 transition-all ${error ? "bg-error" : "bg-[#1CBF74]"} text-white text-sm rounded-md flex items-center justify-between px-4`}>
